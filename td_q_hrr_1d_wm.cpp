...
void printActionValues( WorkingMemory &wm, int episode, vector<string> &metas, int num_states )
{
    ofstream r;
    r.open(action_results_file);
    ...
    for( int j=0; j < metas.size(); ++j )
    {
        ...
        for( int i = 0; i < num_states; ++i )
        {
            ...
            double val = wm.findValueOfStateContentsAction( state, contents, "l" );
            r << val << " ";
        }
        r << endl;
        for( int i = 0; i < num_states; ++i )
        {
            vector<string> state = vector<string>{ to_string(i) };
            double val = wm.findValueOfStateContentsAction( state, contents, "r" );
            r << val << " ";
        }
        r << endl;
    }
